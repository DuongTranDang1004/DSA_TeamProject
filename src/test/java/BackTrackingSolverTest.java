...
public class BackTrackingSolverTest {
    ...
    @Test
    void testSolveTimeoutEventuallyThrows() {
        int[][] board = new int[N][N];
        board[0][0] = 1; // make it hard enough to delay

        BackTrackingSolver solver = new BackTrackingSolver(N, false);
        solver.setTimeoutMillis(1); // Force timeout

        assertThrows(RuntimeException.class, () -> solver.solve(board));
    }
}
